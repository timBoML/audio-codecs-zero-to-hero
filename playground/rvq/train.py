...
def train_step(batch, codec, optimizer):
    ...
    total_loss = loss_time + frequency_loss + commitment_loss
    total_loss.backward()
    grad_norm = torch.nn.utils.clip_grad_norm_(codec.parameters(), max_norm=10.0)
    optimizer.step()
    optimizer.zero_grad()
    return loss_time.item(), frequency_loss.item(), commitment_loss.item(), grad_norm
...
for epoch in range(25):
    ...
    for batch in pbar:
        loss_time, loss_frequency, loss_commitment, grad_norm = train_step(batch, codec, optimizer)
        pbar.set_postfix(
